...
class Attention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        key = self.encoder_att(encoder_out)
        query = self.decoder_att(decoder_hidden)

        atention = self.full_att(self.relu(key + query.unsqueeze(1))).squeeze(2)

        probabilities = self.softmax(atention)

        attention_weighted_encoding = (encoder_out * probabilities.unsqueeze(2)).sum(dim=1)
...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, attention_dim, decoder_dim, embed_dim, vocab_size, encoder_dim=512, dropout_rate=0.5):
    ...
    def forward(self, word, decoder_hidden_state, decoder_cell_state, encoder_out):
        embeddings = self.embedding(word)  # (batch_size, max_caption_length, embed_dim)
